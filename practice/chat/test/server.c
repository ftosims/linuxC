...
void
writeto(char *sender , char *receiver , char *str)
{
    int i;
...
void
member_ctrl(char *mod , char *name)
{
    int i , j , man_loc;
    name = trim(name);
...
recv_msg(void *num)
{
    ...
    while(1){
        ...
        if(!len){
            ...
            if(!strcmp(g_cmd , PUBC_MSG)){
            ...
            } else if(!strcmp(g_cmd , PRVT_MSG)) { // 更改为好友模式
                sscanf(msg_str + readed , "%s%n" , man , &ch_cnt);
                readed += ch_cnt;
                if(judge_group(man)) {
                    //群聊策略; 也既，judge函数是群聊返回1,
                }
            ...
            } else if(!strcmp(g_cmd , LOG)) { //登录－服务端操作
                int rtn = 0;
                char pwdc[20];
                char sqlqwq[200];
                int tr = 0;
                ...
                //数据库认证登录　待写
                sprintf(sqlqwq,"SELECT pwd FROM userid WHERE nick='%s';",client[idx].name);
                printf("%s\n", sqlqwq);
                ...
                if(row[0] == NULL || strcmp(pwdc,row[0])) {
                        send(client[idx].fd, denie, sizeof(denie), 0);
                        continue;
                }
                ...
                member_ctrl(INI_LIST , client[idx].nare);
            } else if(!strcmp(g_cmd, REG)) {
...
int
judge_group(char *obj)
{
    char words[100];
    sprintf( words, "select * from groupid where name = '%s'", obj);
    MYSQL_RES *result;
    MYSQL_ROW row;
    int tr = mysql_real_query(mysql, words, sizeof(words));
    result = mysql_store_result(mysql);
    row = mysql_fetch_row(result);
    if( row == NULL) {
        mysql_free_result(result);
